...
echo ""
echo "Still open for full H7 (todo.md §14): os_entropy_is_deterministic (a static C probe calling"
echo "getrandom()/reading /dev/urandom, byte-identical across two boots) and a guest-driven-"
echo "checkpoint double_boot_ram_hash_identical (see linux-guest/BUILD.md's 'known, deliberate"
echo "non-goal' section for why a raw console/RAM comparison across full boots is not it)."
