...
# drive/h/h5.sh — H5 drive script: snapshot / restore (todo.md §10's H5, first slice)
#
...
# drive/h/h0.sh through drive/h/h4.sh).
# ---------------------------------------------------------------------------
...
SNAP_ROOT="$(mktemp -d -t baud-h5-snap-XXXXXX)"
...
for _ in $(seq 1 60); do
    curl -sf "$SRV/health" > /dev/null 2>&1 && break
    sleep 0.2
done
...
# `--include-ignored` (not `--ignored`): this test is #[ignore]d in-tree so that it runs exactly
# once — here — instead of also inside `cargo test --workspace` (~65s each). `--include-ignored`
# runs it whether or not the attribute is present; `--ignored` against a NON-ignored test reports
# "test result: ok. 0 passed", which the grep below would accept as a false pass. The extra
# non-zero-pass-count assertion turns a zero-tests-ran outcome into a hard failure either way.
# `--test-threads=1` keeps libtest from running other tests alongside it; the test parallelises
# its own 1000 branches internally across BRANCH_WORKERS threads.
BRANCH_OUT=$(cargo test -q -p baud-multiverse thousand_branches_are_independent_and_deterministic -- --include-ignored --test-threads=1 2>&1)
echo "$BRANCH_OUT"
echo "$BRANCH_OUT" | grep -q "test result: ok" || fail "H5.5: thousand_branches_are_independent_and_deterministic FAILED"
echo "$BRANCH_OUT" | grep -qE "test result: ok\. [1-9][0-9]* passed" \
    || fail "H5.5: thousand_branches_are_independent_and_deterministic ran 0 tests (filtered out, never executed) — a false pass"
