...
# drive/m/m8.sh — M8 drive script: Mario under the hypervisor
#
# Validates:
...
#
# This drive requires the real Mario workload. Missing ROM/runtime/frame output is a hard
# failure, because a simulated run cannot prove the guest contract.
...
# ---------------------------------------------------------------------------
# M8.6 — stream frames: real frame hashes stored for main run
# ---------------------------------------------------------------------------
...
[[ "$FRAME_COUNT" -gt "0" ]] || fail "M8.6: real Mario guest produced no frame records"
...
[[ -n "$FIRST_HASH" ]] || fail "M8.6: first real frame has no hash"
pass "M8.6: stream frames — $FRAME_COUNT real frame hashes stored (first: ${FIRST_HASH:0:20}...)"
...
[[ -n "$TAPE_ID" ]] || fail "M8.8: tape create returned no identifier: $TAPE_OUT"
RECON_OUT=$(BAUD_SERVER=http://127.0.0.1:7734 $BAUD tape reconstruct "$TAPE_ID" --json 2>&1)
RECON_OK=$(echo "$RECON_OUT" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('ok',d.get('reconstructed',False)))" 2>/dev/null || echo "False")
[[ "$RECON_OK" == "True" ]] || fail "M8.8: tape reconstruct returned ok=false: $RECON_OUT"
pass "M8.8: tape reconstruct: ok (tape $TAPE_ID)"
