...
# drive/m/m13.sh — M13 drive script: a resume-originated run also gets a real, replayable
...
# run (drive/m/m12.sh), /run/kvm/resume never boots a kernel at all — it restores a Universe straight
...
#         drive/m/m11.sh's plain /run/kvm path and drive/m/m12.sh's branch path both prove, now reached
...
SNAP_ROOT="$(mktemp -d -t baud-m13-snap-XXXXXX)"
