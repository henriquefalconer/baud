...
# drive/pkg/pkg-build-cli.sh — `baud image build` end-to-end, through the real CLI/server (todo.md
...
#
# drive/pkg/pkg-image-build.sh already proves `baud_packages::kernel_build::build_bzimage` itself is
...
# not part of the standard h0-h7 gate, same opt-in convention as drive/pkg/pkg-image-build.sh.
...
# No `pkill -f baud-server` here: `-f` matches whole command lines, so it also killed
# sibling `cargo build -p baud-server` runs and the rustc compiling them. cleanup() already
