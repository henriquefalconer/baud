...
# drive/pkg/pkg-virtio-rng-replay-cli.sh — proves stream::render's real-replay path (reboot
...
# drive/m/m11.sh's own baseline, proving the wiring doesn't perturb unrelated boots.
#
...
SNAP_ROOT="$(mktemp -d -t baud-pkg-rng-replay-snap-XXXXXX)"
...
echo ""
echo "The restore-replay sub-path (resume-originated runs) and /run/kvm/branch|/run/kvm/resume"
echo "now also accept virtio_rng, via a new run_until_branch_or_halt_with_virtio_rng combinator"
echo "in baud-multiverse — see drive/pkg/pkg-virtio-rng-branch-resume-cli.sh."
