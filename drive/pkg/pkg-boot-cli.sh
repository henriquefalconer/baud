...
# drive/pkg/pkg-boot-cli.sh — `baud run kvm --initramfs ... --periodic-timer-period-rcb ...`
...
#
# drive/pkg/pkg-build-cli.sh already proves `baud image build` produces a real bzImage +
# initramfs.cpio.gz pair over the CLI/server; drive/h/h7.sh already proves the real linux-guest
...
# without periodic timer injection, unlike every hand-assembled fixture the pre-existing drive/m/m9.sh
...
# drive/pkg/pkg-build-cli.sh / drive/pkg/pkg-image-build.sh this runs in seconds, not minutes — but it is
...
SNAP_ROOT="$(mktemp -d -t baud-pkg-boot-cli-snap-XXXXXX)"
SERVER_PID=""
