HARD STOP: one iteration = one forward step against @todo-build.md, where a step is the batch of the 10 most important items. A step is complete only when every chosen item is fully implemented in code, fully integrated through its real end-to-end path, and the verification protocol defined in @todo-build.md reports green on the pushed main tip. Audits, scope judgments, large diffs, dependency work, failing tests, missing plumbing, or the fact that an item is difficult are never reasons to abandon a selected item, leave it open, split off a partial fix, or emit NEXT. Keep working on the selected batch until it is complete. Do not give up under any circumstances merely because the work is difficult, broad, slow, or currently failing. Continue debugging, implementing dependencies, and validating until the real acceptance path passes. When the step is closed, append the final progress block, emit a single promise (`<promise>NEXT</promise>` if @todo-build.md still has pending items, `<promise>COMPLETE</promise>` if it has zero pending items), and stop. Owner-blocked points live in `todo-submittal.md`, never in @todo-build.md, and never count as pending.

**Selection lock.** At the beginning of the main session, record the exact ten entries selected from `@todo-build.md`. They are this iteration's binding contract. Do not substitute a smaller slice, relabel a partial implementation as the item, defer a hard subproblem, or rank the same still-pending item again in a new iteration. Audit reports may explain the work, but they do not authorize stopping when the repository still has an implementation path. Before the final progress block, commit, or promise, re-read `@todo-build.md` and check every entry you selected. Every selected entry must now be completely addressed, integrated, tested, and collapsed to its own `DONE` line. If any selected entry is still pending or lacks `DONE`, continue the same session, debug the root cause, and finish it. Only after all selected entries are fully implemented and validated by the existing procedure may you append the final block, commit, push, and return a promise tag.

0a. Study `specs/*` with multiple subagents to learn the application specifications.
0b. Study @todo-build.md.

1. Pick the 10 most important items to address from @todo-build.md (fewer only if @todo-build.md has fewer than 10 pending items; owner-blocked points sit in `todo-submittal.md`, never in @todo-build.md, and are never eligible or counted). Work every selected item in this iteration, resolving each completely before moving to the next. Do not reinterpret "complete" as a partial fix, a library-only change, a fixture, a simulated path, a stub, a fallback, or a task that merely has a plan. Before making changes, search the codebase with subagents (don't assume something is not implemented). Use multiple subagents for searches/reads and a single subagent for build/tests. Use subagents for complex reasoning (debugging, architectural decisions).
2. Author property-based or unit tests, whichever is best.
3. Use chrome use and computer use as needed.
4. Run the tests for the unit of code that was changed, including integration and real end-to-end tests. If functionality is missing, add the complete implementation required by the specifications. Follow every dependency needed by the selected item and finish the real path; do not stop at a partial fix because the remaining work is large or crosses crate, process, guest, host, or packaging boundaries. A failing test starts root-cause work, not abandonment. Ultrathink.
...
6. Before committing, run the pre-push validation @todo-build.md defines, exactly as it defines it. Iterate on root causes until it is green. Do not bypass hooks, weaken the commands, substitute your own sequence, declare an item partial, or give up because a check exposes unfinished work. Give it a generous `timeout: 600000`; if it cannot finish inside one call, re-run it with `run_in_background: true` rather than splitting it into pieces that lose the run. A reported failure is not automatically a regression. Re-run that unit in isolation, repair the cause, and repeat the full validation. Report a flake only after the required evidence, and never work around a failure that reproduces.
...
- `<promise>NEXT</promise>` only after every item selected for this iteration is complete and verified, when @todo-build.md still has pending items for a later batch. Never use `cannot-complete`, audit conclusions, or a failing verification result as permission to stop early. Continue working until every selected item is complete. The outer loop is for the next batch, not for abandoning unfinished work from this batch.
...
Owner-blocked points are recorded in `todo-submittal.md` and deleted from @todo-build.md, so they count towards neither promise — counting a decision you are not allowed to make as pending answers `NEXT` forever on iterations that can only re-read it.
