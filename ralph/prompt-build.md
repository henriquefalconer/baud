...
6. Before committing, run the pre-push validation @todo.md defines, exactly as it defines it. Iterate on root causes until it reports no FAIL. Do not bypass hooks, weaken the commands, or substitute your own sequence for the one @todo.md specifies. Give it a generous `timeout: 600000`; if it cannot finish inside one call, re-run it with `run_in_background: true` rather than splitting it into pieces that lose the run. **Run it once per attempt.** A reported failure is not automatically a regression, and the gate already re-runs the documented rdtsc load-flake in isolation for you, marking those units FLAKE rather than FAIL — a run whose only non-passing units are FLAKE has had that check done for it and counts as validated, so go straight to committing. Never re-run the whole gate to confirm a flake or to hope for a cleaner roll; re-run a unit in isolation only for a failure the gate did not already classify. Report a flake as a flake with both results, and never work around a failure that reproduces. A `ralph/ralph` and `claude -p` pair in `ps` is the loop that launched you, not a competing session — do not investigate it.
