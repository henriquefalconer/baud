...
- **G4 complete deterministic driver tactics and scheduling.** Extend `crates/baud-driver/src/lib.rs` to apply configured input/weather tactics and add deterministic grid-bucket scheduling while preserving replay, reservoir bounds, shrinking, and state resume. Add tactic, grid-balance, bounded-growth, and same-seed tests and run the driver workspace tests plus `drive/m/m3.sh`; invalid parameters must fail deterministically or use a documented neutral path, never panic.
...
- **G8 production host safety contract.** Complete `crates/baud-host/src/{lib.rs,linux.rs,placement.rs}`, doctor routes/CLI, `specs/baud-host.md`, and a focused host drive to expose cooperative/enforced/rejected regime JSON, measure socket/NUMA topology, enforce actual sibling-safe affinity and NUMA placement, honor inherited masks and housekeeping reservations, and report capability failures in `baud doctor`. Run host probe JSON, `doctor_checks_kvm`, `capacity_refuses_sibling_split`, rejected-host tests, and accepted/over-capacity real placement; affinity, NUMA, PMU, or module failures must reject with remediation, never downgrade silently.

- **G8 relocate distributed validation images.** Move authoritative distributed validation image metadata and drives under `examples/`, remove production-drive dependence on `crates/baud-multiverse/tests/fixtures/linux-guest`, and preserve the generic-core guard across `drive/h/*`, `drive/pkg/*`, and `drive/gate.sh`. Acceptance is two byte-identical image builds, the maintained H-series boot drive, and a clean `no_workload_specifics_in_core` check; missing pipeline prerequisites must fail explicitly rather than fall back to crate fixtures.
