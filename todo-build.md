...
- **G8 host module.** Affected paths: `infra/nixos-modules/baud-host.nix`, `infra/machines/`, `specs/baud-host.md`, and host doctor/probe integration. Add a reproducible NixOS host module and machine definition for KVM access, Intel/nested settings, housekeeping and CPU-isolation parameters, and documented secret/store paths, then align doctor/probe diagnostics with those assumptions. Acceptance: `doctor_checks_kvm`, `capacity_refuses_sibling_split`, the generic-core guard, security checks, and `bash drive/gate.sh` pass; missing KVM, unsafe topology, unstable PMU/TSC, missing secrets, or invalid store configuration fails explicitly. Queue after the G1 integration and its prerequisite host capability contract.
