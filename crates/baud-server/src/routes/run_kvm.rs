...
///
/// This does **not yet** let a caller keep *exploring further* from an interesting branch by
/// handing it a fresh tape suffix through this route: every guest fixture this route's own tests
/// use (`hello-guest`, `tape-echo-guest`) halts as soon as it has consumed its given tape
/// (todo.md §3.6's subtractive rule — a minimal guest with no reason to read more), so a node
/// persisted from one of *those* fixtures' branches already has its vCPU in `Hlt`. Forking it with
/// a new suffix and running to first halt is provably a no-op for such a node: found live while
/// building this feature — a first attempt tried to snapshot a branch *after* `run_to_first_halt()`
/// and recursively generate further branches from that post-halt state, and a test proved the
/// resumed branch reproduced the **original** branch's frozen console output, completely
/// independent of the brand-new suffix handed to it (a guest that is already halted never reads
/// anything further).
///
/// **The underlying architectural blocker this implied — no guest that keeps running past a
/// branch point, and no `Multiverse` primitive that stops there instead of at `Hlt` — is now
/// closed** (the "M-series sixth brick" entry's own named prerequisite, todo.md §14): a new
/// fixture, `tests/fixtures/mark-branch-guest/`, calls the tape device's `MARK_BRANCH` control op
/// mid-execution and keeps reading tape/echoing afterward instead of halting, and
/// `Multiverse::run_until_branch_or_halt` (`crates/baud-multiverse/src/linux/mod.rs`) stops a run
/// there instead of only at `Hlt`. Three new `baud-multiverse` tests prove forking a
/// `MARK_BRANCH` checkpoint with a new tape suffix genuinely diverges (the exact no-op finding
/// above, now refuted for a guest that actually has more to read) and that this composes across
/// multiple checkpoint levels. `observations_from_records` still deliberately ignores
/// `Msg::MarkBranch` for scoring (that has not changed and does not need to). **Still open, and
/// specifically what remains for *this route***: nothing here yet calls
/// `run_until_branch_or_halt` instead of `run_to_first_halt`, so a branch generated by this route
/// still always runs a fixture to completion rather than stopping at an intermediate checkpoint to
/// persist and explore further from; wiring that in (deciding when a `MarkBranch` stop counts as
/// its own persistable, further-forkable node, alongside today's goal/crash `interesting` check)
/// is the next concrete step, now unblocked at the primitive level.
