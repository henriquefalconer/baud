...
# Wrapping mechanics (header/ENTRY_OFFSET) are identical to ../hello-guest/build.py,
# ../tape-echo-guest/build.py, and ../rdrand-guest/build.py -- only the assembled payload, and the
# extra IDT-gate byte patching below (needed because GAS/ld cannot resolve a bitwise mask/shift of
# an unresolved relocatable symbol, only a plain add/subtract), differ.
